  ...
  def exculde_passed_events_listings(listings)
    ...
    listings.each do |listing|
      if listing.event.present?
        if listing.event.try(:end_at).present? and listing.event.end_at.strftime("%d/%m/%Y") >= DateTime.now.strftime("%d/%m/%Y") and listing.event.end_at_time.strftime("%H:%M:%S") >= Time.now.in_time_zone("Pacific Time (US & Canada)").strftime("%H:%M:%S")
