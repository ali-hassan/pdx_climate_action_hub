module ListingIndexService::Search::DatabaseSearchHelper
  ...
  def exculde_passed_events_listings(listings)
    ...
    listings.each do |listing|
      if listing.event
        if !listing.event.try(:end_at).nil? and listing.event.end_at.strftime("%d/%m/%Y") >= DateTime.now.strftime("%d/%m/%Y") and listing.event.end_at_time.strftime("%H:%M:%S") >= Time.now.strftime("%H:%M:%S")
